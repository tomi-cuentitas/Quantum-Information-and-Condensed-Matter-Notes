...
\begin{equation}
    \begin{array}{c}
         ...
         \bmt_{N,a} = \prod^{j=N}_{1} \left(\begin{array}{cc}
            u \mathds{1}_j + i \spin_N^z & i \spin_j^-  \\
            i \spin_j^+ & u \mathds{1}_j - i \spin_N^z
...
\begin{align*}
 & & {\br_{a_1, a_2}(u_1-u_2) \bm{\mathcal{T}}_{N, a_1} (u_1) \bm{\mathcal{T}}_{N, a_2} (u_2)}{= \bm{\mathcal{T}}_{n, a_1} (u_2) \bm{\mathcal{T}}_{n, a_1} (u_1) \bm{\mathcal{R}}_{a_1, a_2}(u_1-u_2)}.   
\label{fundamental commutation relation}
\end{align*}

 \blanky \\

Now, let the transfer $\mathfrak{t}$-matrix be defined as the $\bm{\mathcal{T}}$-matrix's partial trace over the axuliary space $\vds_{a}$, ie.
