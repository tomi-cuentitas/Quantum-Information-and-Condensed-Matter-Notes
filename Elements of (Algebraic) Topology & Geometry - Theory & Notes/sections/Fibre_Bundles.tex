...
Two bundles $\fibrebundle{E'}{\pi'}{M}$ and $\fibrebundle{E}{\pi}{M}$ are equivalent if there exists a bundle map $\bar{f}: M \rightarrow M$ s.t. the induced mapping $f: M \rightarrow M$ is the identity map.
...
A \textswab{pullback bundle} can be defined via the the pair $(E,f)$, which defines a new fibre bundle with base-space $B'$ and the same fibre $F$, namely $\fibrebundle{f^{*}E}{\pi_1}{B'}$.
...
The commutativity of the diagram follows since 
...
Moreover, the transition function $t_{ij}$ at $p \in U_i \cap U_j$ maps $f_i$ to $f_j = t_{ij}(f(p)) f_j$.
...
\begin{df}
    ...
    Let $f$ and $g$ be maps from $M'$ to $M$. These are said to \textswab{homotopic} if there exists a smooth map $F: M' \times \R_{[0,1]} \rightarrow M$ s.t. $F(p,0) = f(p)$ and $F(p,1) = g(p)$ for any $p$ in $M'$. 
...
A \textswab{vector bundle} $\fibrebundle{E}{\pi}{M}$ is a fibre bundle whose fibre is a vector space. 
Let $F = \R^k$ and $M$ be an $m$-dimensional manifold. 
Then, $k \equiv \dim E$ is the \textswab{fibre dimension}, with the total space being $(m+k)$-dimensional. 
...
\begin{enumerate}
    ...
    This vector space $N_p M$ is, then, isomorphic to $\R^k$. The \textswab{normal bundle}
...
In particular, a vector bundle whose fibre is one-dimensional, eg. $F = \R$ or $\C$, is called a \textswab{line bundle}.
...
In particular, one may be interested in considering the \textswab{canonical line bundle} $L$. 

\begin{mdframed}[style=MyFrame]

The \textswab{complex projective space} $\C P^n$ is defined similarly to the \textswab{real projective space} $\R P^n$. 
...
The $(n+1)$ numbers $z^0, z^1, \cdots, z^n$ are called the \textswab{homogeneous coordinates}, which is denoted by $[z^0, z^1, \cdots, z^n]$ where $(z^0, z^1, \cdots, z^n) \sim (\lambda z^0, \lambda z^1, \cdots, \lambda z^n)$, with non-zero $\lambda$. 

Then, a chart $U_{\mu}$ is a subset of $\C^{n+1} - \{0\}$ s.t. $z^\mu \neq 0$. In a chart $U_{\mu}$, the \textswab{inhomogeneous coordinates} are defined by 
