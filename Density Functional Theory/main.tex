...
\begin{equation}
    P(\x, \x') = N(N-1) \int_{\Omega \subset \R^{3N}} \prod_{i=3}^{N} d^{3}\x_i \prod_{j=3}^{N} \sum_{\sigma_j \in \mathds{C}^2} |\Psi(\x, \x', \tilde{x}^{(3)}\cdots, \tilde{x}^{(N)})|^2,
    \label{Pair density}
\end{equation}
...
$$
n_{xc}(\x, \x') = n_{x}(\x, \x') + n_{c}(\x, \x'), \textnormal{ which } \begin{array}{c}
     n_{x}(\x, \x') \leq 0  \\
     \blanky \\
